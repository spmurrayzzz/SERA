...
import random
import re
from openai.types.chat.chat_completion_message import FunctionCall as Function, ChatCompletionMessage as ChatCompletionMessageToolCall
...
class InstanceStats(PydanticBaseModel):
    ...
    def _total_tokens(self):
        return self.tokens_received + self.tokens_sent
...
class LiteLLMModel(AbstractModel):
    ...
    def _single_query(
        self, messages: list[dict[str, str]], n: int | None = None, temperature: float | None = None, nff=0
    ) -> list[dict]:
        self._sleep()
        ...
        except (litellm.exceptions.ContextWindowExceededError, litellm.exceptions.RateLimitError) as e:
            if isinstance(e, litellm.exceptions.RateLimitError) and ("glm-4." in self.config.name.lower() or "claude" in self.config.name.lower()) and "Qwen" not in self.config.name.lower():
                print(e)
                raise # forces a retry because its an exit_error
        ...
        # NOTE: This section is only for cost calculator errors.
        try:
    ...
    def _query(
        ...
        if n is None:
            return self._single_query(messages, temperature=temperature, nff=nff)
        outputs = []
        # not needed for openai, but oh well.
        for _ in range(n):
            outputs.extend(self._single_query(messages, nff=nff))
        return outputs

    def query(self, history: History, n: int = 1, temperature: float | None = None, nff=0) -> list[dict] | dict:
        ...
        # NOTE: retries is retries of litellm queries. requeries is how many attempts if the output
        # is bad format, context, etc. Here, we only retry the litellm query if its not one of the key
        # errors. This isn't set in any of the configs, so is default 20 as in RetryConfig().
        for attempt in Retrying(
            ...
            with attempt:
                result = self._query(messages, n=n, temperature=temperature, nff=nff)
        if n is None or n == 1:
            return result[0]
        return result
