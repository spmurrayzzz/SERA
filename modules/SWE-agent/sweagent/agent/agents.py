...
AgentToolConfig = DefaultToolConfig
...
def get_tool_from_config(config: AgentToolConfig):
    if config.type == "default":
        return DefaultTool.from_config(config)
    else:
        msg = f"Unknown tool type: {config.type}"
        raise ValueError(msg)
...
class DefaultAgent(AbstractAgent):
    ...
    def messages(self) -> list[dict[str, Any]]:
        ...
        """
        # Explanation: self.messages retrives from the history.
        filtered_history = [entry for entry in self.history if entry["agent"] == self.name]  # type: ignore

        # Chain the history processors
        # Explanation: History processors remove previous cache_control markings and add new ones to the last
        # n_messages IN PLACE.
    ...
    def setup(
        ...
        """
        # Explanation: This is called at start and adds the system message to history.
        output_dir.mkdir(parents=True, exist_ok=True)
        self._problem_statement = problem_statement
        self._env = env
        ...
        if len(prev_history) > 0:
            self.add_prev_to_history(prev_history)
            self._trajectory = prev_trajectory
        ...
        # Explanation: What happens is on self.tools.install() all the specified tools are installed
        # and as part of it, it creates a /root/state.json and adds select env variables to that file.
        # Then, get_state() fetches those environment variables.
        # Explanation: Looking in yaml, each template is defined. next step template is reporting action
        # outputs. instance template is first observation. system template is self explanatory.
        ### Changes
        env_state = self.tools.get_state(self._env)
        if metadata is not None:
            for key in metadata:
                env_state[key] = metadata[key]
        ### Changes
        self.add_instance_template_to_history(state=env_state) # TODO: Can pass in another kwarg here?
        self._chook.on_setup_done()

    ### Changes
    def add_prev_to_history(self, prev_history) -> None:
        for message in prev_history:
    ...
    def add_system_message_to_history(self) -> None:
        ...
        # Explanation: This is only the system prompt. It does not deal with problem statement
        # or package directory. The start is first the system message and then from an observation role
        # the problem.
        assert self._problem_statement is not None
    ...
    def _add_templated_messages_to_history(
        ...
        """
        # Explanation: Takes input templates and keys to fill and fills them. This works for rendering
        # because model action will be cd or view and then output is from "user" role, which renders
        # the observations (can see that this is the objective of the fn).
        messages = []
    ...
    def _add_assistant_message_to_history(
        self, message: str
        ):
            history_item: dict[str, Any] = {
                "role": "assistant",
    ...
    def add_step_to_history(self, step: StepOutput) -> None:
        ...
        # Explanation: Takes step output and calls self._add_templated_messages_to_history
        # with observation as a kwarg. Thus, Template render is called with observation as input.
        # If adding one step, like here, then the templates list is just a list of one element.
        # This is called every in self.step() for every step.
        self._append_history(
            {
                "role": "assistant",
                "content": step.output,
        ...
        elided_chars = 0
        # Explanation: During actual steps, dont use instance or system template
        # but rather the three step templates.
    ...
    def add_instance_template_to_history(self, state: dict[str, str]) -> None:
        ...
        # Removed this check so that can not add the system template for chained agents
    ...
    def get_trajectory_data(self) -> dict[str, Any]:
        ...
        ### Changes
        if hasattr(self.tools, "config") and hasattr(self.tools.config, "tools"):
            attempt_data["tool_config"] = {
                "tools": self.tools.config.tools,
                "use_function_calling": self.tools.config.use_function_calling
            }
        ### Changes
        return attempt_data
    ...
    def forward(self, history: list[dict[str, str]], nff=0) -> StepOutput:
        ...
        try:
            ...
            if self._action_sampler is not None:
                ...
                output = self.model.query(history, nff=nff)  # type: ignore
    ...
    def forward_with_handling(self, history: list[dict[str, str],]) -> StepOutput:
        ...
        while n_format_fails < self.max_requeries:
            try:
                return self.forward(history, nff=n_format_fails)
            ...
            # Errors that cause exit
                ...
                if "must be at least 1, got" in str(e):
                    n_format_fails += 1
                    return handle_error_with_autosubmission(
                        "exit_context",
                        "Exit due to context window",
    ...
    def step(self, tools = {}) -> StepOutput:
        ...
        if len(tools) > 0:
            step_output = self.forward_with_tools(self.messages, tools)
        else:
    ...
    def run(
        ...
        problem_statement: ProblemStatement | ProblemStatementConfig,
        output_dir: Path = Path("."),
        metadata: Optional[dict] = None, # For subtask of finding function based on description
        prev_history: list = [],
        prev_trajectory: list = [],
        tools: dict = {}
    ) -> AgentRunResult:
        ...
        """
        ### Changes
        self.setup(env=env, problem_statement=problem_statement, 
                   output_dir=output_dir, metadata=metadata, prev_history=prev_history, prev_trajectory=prev_trajectory)
        ### Changes
        ...
        while not step_output.done:
            step_output = self.step(tools=tools)
        ...
        ### Changes
        return AgentRunResult(info=data["info"], trajectory=data["trajectory"], agent_history=self.history, tools=self.model.tools.tools)
        ### Changes
